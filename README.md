...
This will land you to the popup window that allow you to select the subscriptions and resource groups that you would like SRE agent to monitor and manage. You can then select the subscriptions and resource groups under the same tenant, So far so good! As a MSP or ISV you have multiple tenants that you are managing via Azure Lighthouse and you need to have SRE agent access to those. 
...
Provide a Name and Description. Select subscription on Delegated scope. Select **+ Add authorization** which will take you to Add authorization window. Select Principal type, I am selecting User for demo purpose. The pop up window will allow Select users from the list.
...
Azure SRE agent requires an Owner or User Administrator RBAC role in order to assign the subscription in the list of managed resources. As per Lighthouse role support Owner role isn’t supported and User access Administrator role is supported ,but only for limited purpose. Refer this [documentation](https://docs.azure.cn/en-us/lighthouse/concepts/tenants-users-roles#role-support-for-azure-lighthouse) for additional information. If role is not defined correctly you might see an error stating: 
**Failed to add Role assignment** “The 'delegatedRoleDefinitionIds' property is required when using certain roleDefinitionIds for authorization.”
...
Here is the sample [ARM template](https://github.com/pranabpm/Azure-SRE-Agent-Resource-Management-Across-Multiple-Tenants/blob/main/ARM%20Template) used for this demo.

Login to the customers tenant and navigate to the **service provides** from the Azure Portal. From Service Providers overview screen select **service provider offers** from the left navigation pane.
